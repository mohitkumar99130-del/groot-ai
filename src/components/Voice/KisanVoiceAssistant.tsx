...
import { getMultilingualVoiceScripts, getLanguageMeta } from '../../services/languageService';
...
export const KisanVoiceAssistant: React.FC<KisanVoiceAssistantProps> = ({
  ...
  // Mock weather for script generation
  const mockWeather = {
    temperature: 28.4,
    humidity: 64,
    windSpeed: 8.8,
  ...
  const [transcript, setTranscript] = useState<string>(scripts.welcome);

  useEffect(() => {
    if (!currentNarrating) {
      setTranscript(scripts.welcome);
    }
  ...
  const getHealthText = () => scripts.health;
  const getFertilizerText = () => scripts.fertilizer;
  const getPestText = () => scripts.disease;
  const getFullSummaryText = () => scripts.fullAudit;
      ...
      <div className="agri-card-elevated p-6 rounded-3xl border border-amber-500/30 text-center max-w-4xl mx-auto space-y-4 shadow-2xl">
        ...
        <div>
          <h2 className="font-display font-black text-xl sm:text-2xl text-white">
            🎙️ {langMeta.welcomePhrase} — Kisan Sahayak AI Voice Narrator ({langMeta.nativeName})
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 font-mono mt-1 max-w-2xl mx-auto">
            {isEn
              ? 'Hands-free multilingual voice assistant for field workers, narrating localized agronomic prescriptions.'
              : `${langMeta.nativeName} (${langMeta.region}) में बिना पढ़े-लिखे किसान भाइयों के लिए सरल आवाज़ में खेत की पूरी सेहत, खाद और दवाई की जानकारी।`}
          </p>
        </div>
        ...
        <div className="bg-slate-950/90 p-4 rounded-2xl border border-amber-500/30 text-left space-y-2">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400 border-b border-slate-800 pb-2">
            <span className="flex items-center gap-2 text-amber-300 font-bold">
              ...
              {currentNarrating ? `AUDIO NARRATION PLAYING (${langMeta.nativeName.toUpperCase()})...` : `VOICE TRANSCRIPT READY (${langMeta.nativeName.toUpperCase()})`}
            </span>
          ...
          {currentNarrating && (
            <div className="flex items-center gap-1 pt-2">
              ...
              <span className="text-[11px] font-mono text-amber-300 ml-2">Indian Regional AI Speech Synthesizer ({langMeta.speechCode})</span>
            </div>
          )}
        </div>
          ...
          >
            ...
            <div>
              <div className="text-xs font-mono font-bold text-emerald-400">
                1. {isEn ? 'Listen Crop Vitality' : 'फसल की सेहत सुनें'}
              </div>
              <div className="text-sm font-bold text-white mt-0.5">
                {isEn ? 'Crop Health & Vitality' : 'फसल स्वास्थ्य एवं जीवन शक्ति'}
              </div>
          ...
          >
            ...
            <div>
              <div className="text-xs font-mono font-bold text-amber-400">
                2. {isEn ? 'Listen Fertilizer Dosage' : 'खाद की मात्रा सुनें'}
              </div>
              <div className="text-sm font-bold text-white mt-0.5">
                {isEn ? 'Fertilizer Prescription' : 'खाद खुराक सिफारिश'}
              </div>
              <div className="text-[11px] text-slate-400 mt-1">
          ...
          >
            ...
            <div>
              <div className="text-xs font-mono font-bold text-cyan-400">
                3. {isEn ? 'Listen Disease Remedy' : 'बीमारी की दवा सुनें'}
              </div>
              <div className="text-sm font-bold text-white mt-0.5">
                {isEn ? 'Antifungal & Pest Remedy' : 'फंगल व कीट उपचार'}
              </div>
              <div className="text-[11px] text-slate-400 mt-1">
          ...
          >
            ...
            <div>
              <div className="text-xs font-mono font-black text-slate-900">
                4. {isEn ? 'Full Spoken Audit' : 'संपूर्ण रिपोर्ट सुनें'}
              </div>
              <div className="text-sm font-black text-slate-950 mt-0.5">
                {isEn ? '360° Multimodal AI Summary' : '360° समग्र AI सारांश'}
              </div>
              <div className="text-[11px] text-slate-900/80 mt-1">
                {isEn ? 'Satellite + Soil + Leaf Diagnostics' : 'उपग्रह + सेंसर + कैमरा विश्लेषण'}
              </div>
            </div>
          </button>
